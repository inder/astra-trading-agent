...
export function createAgentMcpServer(service: TradingAgentService): McpServer {
  ...
  server.registerTool("list_accounts", { description: "List the user's Robinhood accounts so they can choose which to report on. Returns a masked label and an opaque handle for each; account numbers are never returned. Reads account names and status only — no balances, no positions, no orders. Show the labels and ask which accounts the user wants before reading any of them.",
