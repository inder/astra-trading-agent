...
export function createAgentMcpServer(service: TradingAgentService): McpServer {
  ...
  server.registerTool("get_levels", { description: "Support and resistance for stocks, computed from Robinhood price history: price zones with how often they were tested, open gaps, trend lines and moving averages. A zone the price closed through by a margin, for enough settled sessions, carries `broke`: which way it went, the session it went on, how many closes since, and how often that zone had turned the price away BEFORE it went (`testsBefore` \u2014 not the same number as `tests`, which is measured under the side the zone is on NOW). `overhead` counts what stands above the price, so \"nothing above it\" can be stated rather than left blank. A break is a record of a settled session and does not move intraday; `overhead` follows the price shown. Daily bars cover the quarter, the year and two years; ask for the \"5y\" timeframe and it measures five years of WEEKLY bars instead, for the long view. Reads market data only, never an account. Advisory: it describes what the rules found, never what to buy or sell.",
