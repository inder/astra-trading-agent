...
export async function verifyInstallation() {
  ...
  const transport = new StdioClientTransport({ command: process.execPath,
    args: [join(root, "src", "agent-server.ts"), "--data-dir", directory], env: { PATH: process.env.PATH ?? "" }, stderr: "pipe" });
  try {
    ...
    const sample = await call("run_sample", { strategyId: "opening-range-options", symbols: ["DEMOA", "DEMOB"], includePremarket: false, requestId: "installation-check" });
    ...
    return { verified: { node: process.versions.node, localMcpHandshake: true, syntheticSample: true, strategyCount: catalog.strategies.length },
  ...
  } finally { try { await client.close(); } finally { rmSync(directory, { recursive: true, force: true }); } }
}
