...
import { chromium, firefox, webkit, type Browser, type BrowserType, type Page } from "playwright-core";
...
// Read the POST response itself, not the rendered page, so the check never races the navigation.
...
for (const type of [chromium, webkit, firefox]) {
  test(`${type.name()}: human reject, cross-site submits with the real token refused, then human approve`, async t => {
    ...
    // Cross-site: loading the real page gives this browser the review cookie; the attack pages on another
    // loopback port reuse the REAL form token. Other ports on 127.0.0.1 are the same site, so the
    // SameSite=Strict cookie is sent too — the exact-origin check is the only thing that can refuse them.
    // The no-referrer variant makes the browser send `Origin: null`, guarding against a future
    // "allow null origins" fix that would reopen the door.
    ...
    const attacker = createServer((req, res) => res.writeHead(200, { "Content-Type": "text/html" }).end(
      (req.url === "/no-referrer" ? `<meta name="referrer" content="no-referrer">` : "") +
    ...
    for (const [path, expectedOrigin] of [["/", attackerOrigin], ["/no-referrer", "null"]] as const) {
      ...
      const [attack] = await Promise.all([
        ...
        attackPage.goto(attackerOrigin + path),
      ]);
      const sent = await attack.request().allHeaders();
      assert.equal(sent.origin, expectedOrigin, `browser must report origin ${expectedOrigin} for ${path}`);
    ...
    assert.equal(await again?.text(), "Review already executed.");
  });
}
